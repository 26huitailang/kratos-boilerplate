...
func (m *SwaggerUIMiddleware) Handler() http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if m.config.EnableAuth {
			m.logger.Warn("Swagger UI authentication is currently disabled")
			// TODO: 实现认证检查
		}
...
// checkAuthentication 检查用户认证 (暂时禁用)
func (m *SwaggerUIMiddleware) checkAuthentication(r *http.Request) error {
	// TODO: 实现认证检查
	m.logger.Warn("Authentication check is not implemented yet")
	return nil
}
...
func RegisterSecureSwaggerUI(
	...
	middleware := NewSwaggerUIMiddleware(config, logger)
